...
const NoticeDetail = ({history, match}) => {
    ...
    const getTotal = () => {
        axios.get(`https://server.imthesong.site/notice/detail/${match.params.id}`)
            .then(({data}) => {
    ...
    const onClickComment = e => {
        e.preventDefault()
        axios.post(
            `https://server.imthesong.site/noticecomment/save`,
    ...
    const onChangeNoticeDelete = value => {
        if (value !== password) {
            ...
            setIsPwOk(true)
            setTimeout(() =>
                axios.get(`https://server.imthesong.site/notice/delete/${match.params.id}`)
                    .then(() => {
                        history.goBack()
                    }), 800)
        }
    ...
    const onClickCommentDelete = commentNo => {
        axios.get(`https://server.imthesong.site/noticecomment/delete/${commentNo}`)
            .then(() => {
                getTotal()
            })
    }
    ...
    const onClickDownload = (id, fileName) => {
        axios.get(`https://server.imthesong.site/file/download/${id}`, {
