...
const Mypage = ({history}) => {
    ...
    useEffect(() => {
        axios.get(`https://server.imthesong.site/user/find/${localStorage.getItem("userCode")}`)
            .then(({data}) => {
                setName(data.name)
    ...
    const onSubmit = (userId, schoolCode, schoolName, curGrade, homeClass, phone, email) => {
        axios.post(
            'https://server.imthesong.site/user/update',
            {
    ...
    const onChangePassword = value => {
        if (value.length != 0) {
            axios.get(`https://server.imthesong.site/user/check/${localStorage.getItem("id")}/${value}`)
                .then(({data}) => {
                    if (data === 'FAIL') {
                        setIsPwFail(true)
                        setIsOldPw(true)
    ...
    const onClickUpdatePw = () => {
        axios.get(`https://server.imthesong.site/user/update/${localStorage.getItem("id")}/${newUserPw}`)
            .then(() => {
                setIsChanged(true)
                setTimeout(() => {
                    setIsPwOk(false)
